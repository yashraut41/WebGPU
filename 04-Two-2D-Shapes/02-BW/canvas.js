 ...
 * @modify date 2026-07-29 07:07:44
 * @desc [description]
 */
...
function initialize() {
    ...
    const triangle_vertex_position = new Float32Array([
    ...
    // as only we have one mvp uniform its  bind group layout is common for both triangle and square 
    ...
    //==============================================================================================//
    // triangle: its position, buffer its uniform buffer , its bind  group.
    // postion buffer for triangle
    // 4.Create vertex buffer for postion. GPUBufferDescriptor
    // A. Create buffer descriptor
    const bufferDescriptor_position_triangle =
    {
        size: triangle_vertex_position.byteLength,
    ...
    queue.writeBuffer(buffer_position_triangle, 0, triangle_vertex_position, 0, triangle_vertex_position.length);
    ...
    // create bind group for mvp uniform buffer for traingle 
    ...
    //==============================================================================================//
    // square: its position, buffer its uniform buffer , its bind  group.
    // postion buffer for triangle
    // 4.Create vertex buffer for postion. GPUBufferDescriptor
    // A. Create buffer descriptor
    ...
    if (buffer_position_square == null) {
        console.log("Failed to create vertex buffer for position square \n");
        throw Error("Failed to create vertex buffer for position square\n");
    } else {
        console.log("Vertex buffer for sqauare position is created successfully \n");
    }
    ...
    console.log("Writing Vertex position data  into square position buffer  is completed \n");
...
function display() {
    ...
    */
    ...
    mat4.translate(modelViewMatrix, modelViewMatrix, [-1.5, 0.0, -6.0]); //translate the modelview matrix by -4 units in z direction
